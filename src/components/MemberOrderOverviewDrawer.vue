    ...
    methods: {
      content(product_shot) {
        ...
        let ret = items.map(item => {
          return `${item.name}/${item.specification_detail.spec1_name}/${item.specification_detail.spec2_name}/*${item.specification_detail.quantity}/$${item.specification_detail.price}`
        })
